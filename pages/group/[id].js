...
//
// 판정 기준은 '지금 주소에 ?admin=<ADMIN_KEY>가 붙어 있는가' 딱 하나다.
//   진입(북마크 권장): https://bbf-museum.vercel.app/group/87?admin=bbf-admin-2026
//   해제: 주소에서 ?admin=... 를 떼면 즉시 일반 사용자
//
// ⚠️ localStorage에 저장하지 않는다. 저장 방식은 링크를 한 번 누른 순간 그 브라우저의
//    '모든' 방문이 운영자가 되고, 화면만 봐서는 그 사실을 알 수 없어 디버깅이 불가능했다.
//    주소가 곧 상태 = 눈으로 보이는 상태.
...
function GroupMosaic({ groups, paintingSrc, currentGroupId, isAdmin }) {
  ...
  const handleTileClick = (g) => {
    // 일반 참여자: 조 번호·진행률 표시까지만. 타 조로의 이동은 운영자 전용.
    if (!isAdmin) {
      setHoverGroup(g);
...
export default function GroupDashboard() {
  ...
  // 운영자 모드 판정 (mount 1회). 지금 주소에 ?admin=<KEY>가 있으면 운영자, 없으면 아니다.
  // 저장하지 않으므로 /group/59 같은 평범한 주소에서는 구조적으로 운영자가 될 수 없다.
  // 운영자 링크는 '이달의 명화 전시관' 탭으로 시작한다 — 이 링크의 목적이 모자이크 순회이기 때문.
  ...
  // ⚠️ cleanup 필수: 탭·월이 바뀌면 이전 effect가 띄운 이미지의 onload가 나중에 도착해
  //    loadedSrc를 '이전 그림'으로 되돌려 놓는다. 그러면 loadedSrc !== artworkSrc가 되어
  //    revealReady가 영영 false → 그림이 어두운 채로 멈춘다(새로고침해야 보이던 증상).
  //    mount 직후 getDefaultMonth()가 월을 한 번 바꾸므로 첫 진입에서 반드시 재현된다.
  useEffect(() => {
  ...
  //   ※ 2026-08-07부터 진도율·모자이크는 isInactive를 쓰지 않는다(분모 = 현재 명단 전체).
  //     계산은 주석 처리된 흐림/💤/하단정렬 복원용으로만 남아 있다.
  ...
  // ── 진도율 모집단 = 현재 명단 전체 ─────────────────────
  // 7월 이후 중도탈락자를 DB에서 정리했으므로, 뒤처진 사람을 분모에서 빼지 않는다.
  // 활성 멤버 분모를 쓰면 뒤처진 인원이 통째로 빠져 실제로는 100%가 아닌 조가 100%로 보였다.
  // 분자·분모 모두 rosterNames 기준 → §4.4 동일 모집단 불변식 유지 + 일차별 헤더 %와 분모 일치.
  const rosterNames = new Set(members.map(m => m.name)); // 유령 로그 배제(현재 명단만)
  const rosterCount = members.length;
  const groupTargetGoal = rosterCount * targetDays;
  ...
  //   (누적 진도율 §4.4·모자이크도 이제 같은 분모를 쓴다 — 헤더가 전부 100%면 진도율도 100%)
  // rosterNames / rosterCount는 위 진도율 블록에서 이미 선언됨 (같은 분모를 공유한다)
  const dailyCounts = {};
  logs.forEach(l => {
    ...
    if (activeTab === '150일 대장정') {
      ...
      switch(currentMonth) {
        ...
        case '8월': {
          ...
          const aCenter = isExhibit ? '44% 49%' : '69% 42%';
    ...
    <div className="min-h-screen bg-[#0A0A0C] text-[#E4E4E7] p-4 md:p-8 font-sans antialiased selection:bg-[#E67E22]/30">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="text-center mb-10 pt-4">
          ...
          {/* 지금 운영자 모드인지 눈으로 보이게 한다. 저장 상태를 추론할 필요가 없어야 한다. */}
          {isAdmin && (
                  ...
                  <thead>
                    <tr className="bg-[#141416] text-[#52525B] border-b border-[#1F1F23] text-sm font-bold">
                      ...
                      {Array.from({ length: targetDays }).map((_, i) => {
                        ...
                        const isPerfect = !isFutureDay && isFullColumn;                   // 전원 체크 → 금색(완전체)
                        const isNearFull = !isFutureDay && !isPerfect && dayPct >= LIT_THRESHOLD; // 90~99% → 은색
                        const dayColor = isPerfect ? '#FFD700' : isNearFull ? '#D4D4D8' : null;
                        return (
                          <th
                            key={i}
                          ...
                          >
                            <div style={isPerfect
                                ? { color: '#FFD700', textShadow: '0 0 6px rgba(255,215,0,0.9), 0 0 14px rgba(255,179,102,0.6), 0 0 24px rgba(255,179,102,0.35)' }
                                : isNearFull ? { color: '#D4D4D8' } : undefined}>{displayDate}</div>
                            <div style={{ fontSize: '11px', marginTop: '2px', fontWeight: dayColor ? 700 : 400, color: dayColor || '#52525B' }}>
